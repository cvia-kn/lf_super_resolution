...
#
  ...
  def create_2D_decoder( self, decoder_config):
    ...
    with tf.variable_scope( decoder_id ):
      ...
      decoder[ 'bicubic'] = resize_bicubic(self.stack_v[:, self.cv_pos, :, :, decoder[ 'start']:decoder[ 'end']],[self.H_HR,self.W_HR])
  ...
  def setup_losses( self ):
   for id in self.decoders_2D:
      ...
      if self.config.config['ColorSpace']=='YUV':
          ...
          tf.summary.image(id + '_input_bicubic', self.decoders_2D[id]['bicubic'], max_outputs=3)
