...
training_data_filename = 'lf_benchmark_YCbCr.hdf5'
...
for lf_name in data_folders:
    ...
    LF_LR_lab = np.zeros((LF.shape[0], LF.shape[1], int(LF.shape[2] / scale),
                      int(LF.shape[3] / scale), int(LF.shape[4])), np.float32)
    #
    # disp = file_io.read_disparity(data_folder)
    # disp_gt = np.array(disp[0])
    # disp_gt = np.flip(disp_gt, 0)
    cv_gt = rgb2lab(lf_tools.cv(LF))
    ...
    for v in range(0, nviews):
        for h in range(0, nviews):
            ...
            LF_LR_lab[v, h, :, :, :] = rgb2lab(LF_LR[v, h, :, :, :])

    cv1 = lf_tools.cv(LF_LR_lab).astype(np.float64)
    cv2 = lab2rgb(cv1)
    cv3 = lf_tools.cv(LF_LR)
    tst2 = np.sum(np.abs(cv3 - cv2))
    # plt.imshow(cv2)
