...
training_data_dir = "/home/z/PycharmProjects/SR YCbCr/"
training_data_filename = 'lf_patch_synthetic_YCbCr_sr_1.hdf5'
    ...
    for i in range(0, 2):
        if os.path.isdir(os.path.join(data_folder, inner_folder, flow[i])):
            ...
            LF_LR = np.zeros((LF.shape[0],LF.shape[1],int(LF.shape[2]/scale),
                            int(LF.shape[3]/scale),int(LF.shape[4])),dtype=np.float64)
            #
            # disp = file_io.read_disparity(data_folder)
            # disp_gt = np.array(disp[0])
            # disp_gt = np.flip(disp_gt, 0)
            cv_gt = rgb2YCbCr(lf_tools.cv(LF))
            cv_rgb = lf_tools.cv(LF)
            vc = YCbCr2rgb(cv_gt)
            # plt.imshow(vc)
            # plt.show()
            tst = np.sum(np.abs(cv_rgb - vc))
            print(tst)
            #
            for v in range(0, nviews):
                for h in range(0, nviews):
                    ...
                    LF_LR[v, h, :, :, :] = rgb2YCbCr(LF_LR[v, h, :, :, :])

            # cv1 = lf_tools.cv(LF_LR)
            # cv2 = ycbcr2rgb(cv1).astype(np.float32)
            # plt.imshow(cv2)

            # LF_LR = LF_LR.astype(np.float64)
            # LF_LR_new = np.zeros(LF_LR.shape, dtype=float)
            # for v in range(0, nviews):
            #     for h in range(0, nviews):
            #         LF_LR_new[v,h,:,:,:] = rgb2ycbcr(LF_LR[v,h,:,:,:])
