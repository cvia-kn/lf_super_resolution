    ...
    def __init__(self, layer_id, layout, patches, input_features, output_features):
        ...
        with tf.variable_scope(layer_id):
            if not input_features:
                self.shape = [1, 1,
                              int(layout['conv'][4] * 4 * patches),
                              layout['conv'][4]]
            else:
                self.shape = [1, 1,
                              int(input_features * 4 * patches),
                              output_features]
...
class upscale_variables:
    def __init__(self, layer_id, layout, patch_weight, skip_connection, bicubic_features = 0):
        # define variables for standard resnet layer for both conv as well as upconv
        with tf.variable_scope(layer_id):
            self.stride = layout['stride']
            # to be initialized when building the conv layers
            ...
            if skip_connection:
                self.shape = [layout['conv'][0], layout['conv'][1],
                              int(layout['conv'][2] * patch_weight),int(layout['conv'][3]* (patch_weight + 1) + bicubic_features)]
            else:
                self.shape = [layout['conv'][0], layout['conv'][1],
                              int(layout['conv'][2] * patch_weight),int(layout['conv'][3]*patch_weight + bicubic_features)]
